...
def population_map():
    roads = gpd.read_file(
        "zip://safenight/HKU_ROUTE_MEAN_PopDen.zip!HKU_ROUTE_MEAN_PopDen/HKU_ROUTE_MEAN_PopDen.shp"
    )
...
def lamp_map():
    lamps = gpd.read_file(
        "zip://safenight/Lamppost_HKU.zip!Lamppost_HKU/Lamppost_HKU.shp")
    m = folium.Map(location=CENTER, zoom_start=15, tiles=TILES)
...
def cctv_map():
    cams = gpd.read_file(
        "zip://safenight/cctv.zip!CCTV_all_fc/CCTV_all_fc.shp")   # ← 若不对只改这里
    m = folium.Map(location=CENTER, zoom_start=15, tiles=TILES)
